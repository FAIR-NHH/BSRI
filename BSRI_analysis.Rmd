---
title: "Analysis for global evidence on the selfish rich inequality hypothesis"
...
```{r}
WPs <- readRDS(here::here("pnas_BSRI","data","WP_selfishness_confidential.rds"))
```
...
averages_with_se <- estimates %>%
  fatwo_rcfigure(selfish_mean, selfish_se, country,
                 hline=3, path = here::here("pnas_BSRI","graphs","SR_averages_with_se.pdf"))
averages_with_se
```
...
overview_display
ggsave(here::here("pnas_BSRI","graphs","Figure-1.pdf"), width=16, height=12, units="cm")
```
...
histograms1 <- country_bars %>% filter(country %in% countries1) %>%
  ...
  facet_wrap(.~country)
histograms1
ggsave(here::here("pnas_BSRI","graphs","selfishness_histograms1.pdf"), width=24, height = 16, units="cm")
histograms2 <- country_bars %>% filter(country %in% countries2) %>%
  ...
  facet_wrap(.~country)
histograms2
ggsave(here::here("pnas_BSRI","graphs","selfishness_histograms2.pdf"), width=24, height = 16, units="cm")
```
...
```{r}
df_selfishness_c <- haven::read_dta(here::here("pnas_BSRI", "data", "BSRI_external_data.dta")) %>%
...
ggsave(here::here("pnas_BSRI","graphs","belief_classifications_histogram.pdf"), width = 16, height = 16, units = "cm")
```
...
ggsave(here::here("pnas_BSRI","graphs","belief_classifications_ranked.pdf"), width=16, height = 16, units = "cm")
```
...
```
In how many countries is there a at least 20% disagreeing with belief in crime **among** those who believe in SRI?
...
ggsave(here::here("pnas_BSRI","graphs", "Figure-2.pdf"),
...
msummary(list("(1)"=wdi1,"(2)"=wdi2,"(3)"=wdi3,"(4)"=wdi4,"(5)"=wdi5,"(6)"=wdi6, "(7)"=wdi7,"(8)"=wdi8),
         ...
         output="latex") %>% 
    add_header_above(c(" "=1, "Belief in Selfish Rich Inequality"=8)) %>%
  cat(file=here::here("pnas_BSRI","tables","WDI_variables.tex"))
```
...
msummary(list("(1)"=hdi_1, "(2)"=hdi_2, 
  ...
  add_header_above(c(" ", "Including HDI"=2, "Including GPI"=2, "Business corruption"=1, "Government corruption"=1)) %>%
  add_header_above(c(" "=1, "Belief in Selfish Rich Inequality"=6)) %>%
  cat(file=here::here("pnas_BSRI","tables","variant_explanatory_variables.tex"))
```
...
msummary(list("(1)"=lcorruption,"(2)"=lshadow_ec, "(3)"=lorg_crime, "(4)"=lall, "(5)"=lgaincrime),
  ...
  add_header_above(c(" "=1, "Belief in Selfish Rich Inequality "=5)) %>%
  cat(file=here::here("pnas_BSRI","tables","crime_measures.tex"))
...
msummary(bgmodels, title="Associations within countries", 
  ...
  add_header_above(c(" "=1, "Belief in Selfish Rich Inequality"=10)) %>%
  cat(file=here::here("pnas_BSRI","tables","selfishness_within_bg.tex"))
```
...
within_display
ggsave(here::here("pnas_BSRI", "graphs","Figure-3.pdf"), width=16, height=16, units="cm")
```
...
pol_means <- polarization_estimates %>% 
  fatwo_rcfigure(more_selfish_polarization, more_selfish_se, country,
                 ...
                 path = here::here("pnas_BSRI", "graphs","SR_polarization.pdf"))
pol_means
```
...
country_params %>% dplyr::select(iso_a3, term, estimate, std.error) %>%
  ...
  fatwo_rcfigure(estimate, std.error, country, ytitle="Coefficient on income rank \u00B1 s.e.",
                 hline=0, path=here::here("pnas_BSRI","graphs","selfishness_incrank_coef_joint.pdf"))
```
...
msummary(list("(1)"=catt1, "(2)"=catt2, "(4)"=catt4, "(5)"=catt5),
         ...
         output = "latex") %>%
  add_header_above(c(" " = 1, "Inequality is unfair" = 2, "Should reduce inequality" = 2)) %>%
  cat(file= here::here("pnas_BSRI","tables","attitudes_vs_beliefs_countries.tex"))
```
...
df_attitude_c %>% dplyr::select(mean_Bs,
                                ...
                                zSchooling,
                                zCorruption) %>%
  write_csv(file=here::here("pnas_BSRI","SR_df_attitude_c.csv"), na="." )
```
...
msummary(att_all, coef_map = att_names, gof_omit = "R2 Adj.|se_type", 
         ...
         title="Attitudes vs beliefs---within information") %>%
  add_header_above(c(" " = 1, "Inequality is unfair" = 2, "Should reduce inequality" = 2))  %>%
  cat(file= here::here("pnas_BSRI","tables","attitudes_vs_beliefs_individuals.tex"))
```
...
WPsr2 %>% 
  ...
  haven::write_dta(here::here("pnas_BSRI", "WPsr2.dta"))
```
...
msummary(list(religion_r1, religion_r2, religion_r3, religion_r4, religion_r5, religion_r6), 
         ...
         output = 'latex') %>%
  add_header_above(c(" " = 1, "Belief in Selfish Rich" = 2, "Inequality is unfair"=2, "Government should aim..."=2))  %>%
  cat(file= here::here("pnas_BSRI","tables","religion_subsample.tex")) 
```
...
```{r}
ggsave(here::here("pnas_BSRI","graphs","SR_charity_of_rich.pdf"), width = 16, height = 10, units="cm")
```
...
```{r}
scm_df <- read_csv(here::here("pnas_BSRI", "data", "Durante2017.csv"))
```
